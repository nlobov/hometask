...
//            selectExRange();
    ...
    private static void preparedStmtEx(String tableName) throws SQLException {
        long t = System.currentTimeMillis();
        connection.setAutoCommit(false);
        String sql = String.format("INSERT INTO %s", tableName);
        psInsert = connection.prepareStatement("INSERT INTO ? (prodid, title, cost) VALUES (?, ?, ?);");
        for (int i = 0; i < 5000; i++) {
            psInsert.setString(1, "goodsTable");
            psInsert.setString(2, (i + 1) + "1");
            psInsert.setString(3, "car");
            psInsert.setInt(4, 20 + (i * 10) % 90);
            psInsert.addBatch();
        }
